...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { count: rCount } = await supabase.from('ratings').select('*', { count: 'exact', head: true }).eq('ratee_id', user.id)
      setRatingCount(rCount || 0)
    }
    load()
  }, [])
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 text-center shadow-sm">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-400 to-green-500 flex items-center justify-center text-white text-2xl font-bold mx-auto mb-3">
            {initials}
          </div>
          {!editing ? (
            <>
              ...
              <button onClick={() => setEditing(true)}
                className="mt-4 px-5 py-2 text-sm font-semibold text-blue-600 border border-blue-200 rounded-xl hover:bg-blue-50 transition">
                Edit Profile
              </button>
            </>
          ) : (
            <div className="space-y-3 text-left mt-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={saveProfile} disabled={saving}
                  className="flex-1 py-2.5 bg-gradient-to-r from-blue-500 to-green-600 text-white rounded-xl text-sm font-semibold disabled:opacity-50">
                  {saving ? 'Saving…' : 'Save'}
                </button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Exchanges', value: stats.exchanges, emoji: '🤝' },
            { label: ratingCount > 0 ? `Trust · ${ratingCount} rating${ratingCount > 1 ? 's' : ''}` : 'Trust Score', value: `${profile?.trust_score ?? 50}`, emoji: '⭐' },
          ].map(s => (
        ...
        {/* Invite link */}
        <div className="bg-gradient-to-br from-blue-500 to-green-600 rounded-2xl p-5 text-white shadow-sm">
          ...
          <div className="bg-white/20 backdrop-blur rounded-xl px-3 py-2 flex items-center justify-between gap-3">
            ...
            <button onClick={copyInvite}
              className="flex-shrink-0 bg-white text-blue-600 text-xs font-bold px-3 py-1.5 rounded-lg transition active:scale-95">
              {copied ? '✓ Copied!' : 'Copy'}
            </button>
          </div>
        </div>
